...
public class CalculationFormulaControllerTests(ApiFixture fixture) : IClassFixture<ApiFixture>, IAsyncLifetime
{
    private readonly HttpClient _client = fixture.CreateClient();

    private const string CalculationFormulasUrl = "/api/v1/calculation-formulas";
    ...
    [Fact]
    public async Task GetCalculationFormulas_WithoutToken_ShouldReturnUnauthorized()
    {
        var response = await _client.GetAsync($"{CalculationFormulasUrl}?Pagination.PageNumber=1&Pagination.PageSize=10");

        response.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }

    [Fact]
    public async Task GetCalculationFormulas_WithUserWithoutSiteManagerRule_ShouldReturnForbidden()
    {
    ...
    [Fact]
    public async Task CreateCalculationFormula_WithSiteManagerRule_ShouldReturnOk()
    {
    ...
    [Fact]
    public async Task CreateCalculationFormula_WithUserWithoutSiteManagerRule_ShouldReturnForbidden()
    {
        var registerRequest = new RegisterUserCommand("09123456789", "ali@gmail.com", "علی رضایی", "123456");
        var registerResponse = await _client.PostAsJsonAsync(RegisterUrl, registerRequest);
        var registerResult = await registerResponse.ShouldBeSuccess<RegisterUserCommandResponse>();
